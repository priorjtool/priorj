...
//		JavaIO.createXMLFile(dir, "coveragePriorJ.xml", "<list>", true);
	...
	public void initCoverageFile(final String suitepackName, final String coverageFileName) {
		dir = JavaIO.USER_DIR+JavaIO.SEPARATOR+"report"+JavaIO.SEPARATOR;
		...
		dir = dir + suitepackName + JavaIO.SEPARATOR;
		file = dir + coverageFileName;
		if(!JavaIO.exist(dir)) {
			JavaIO.createLocalFolder(dir);
		}
	}
	...
	public void commit() {
		...
		for (TestSuite suite : suites) {
			String suitePackName = suite.getPackageName() + "-" + suite.getName();
			List<TestCase> tcs = suite.getTestCases();
			for (TestCase tc : tcs) {
				String coverageFileName = "coveragePriorJ-" + tc.toString() + ".xml";
				initCoverageFile(suitePackName, coverageFileName);
				JavaIO.createXMLFile(dir, coverageFileName, "<list>", true);
				JavaIO.saveObjectToXML(dir, coverageFileName, tc, true);
				JavaIO.createXMLFile(dir, coverageFileName, "</list>",true);
			}
		}
