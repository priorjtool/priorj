...
public class GenerateTestSuite {
	...
	public static String getSuiteBodyTop(String className) {
		...
		strBody += "\tprivate int firstFailure = 0;" + newline;
...
//	}
	...
	public static String getFinalSuite(String suiteName, List<String> tests) {
		...
		code += "\tif (!result.wasSuccessful() && firstFailure  == 0)" + newline;
		code += "\tfirstFailure = runCount;" + newline;
		code += "\t}" + newline;
		...
		code += "\tSystem.out.println(\"First Failure: \" + this.firstFailure);\n";
		code += "\t}" + newline;
